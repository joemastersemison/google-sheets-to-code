...
import { SheetConfig, Sheet, Cell } from '../types/index.js';
...
describe('Integration Tests', () => {
  ...
  describe('Code Generation Integration', () => {
    ...
    it('should generate valid TypeScript code', () => {
      ...
      expect(code).toContain("cells['Input!C1'] = input.Input.C1 ?? \"Product A\";");
    ...
    it('should generate valid Python code', () => {
      ...
      expect(code).toContain("cells['Input!C1'] = input_sheet.get('C1', 'Product A')");
    ...
    it('should generate executable TypeScript code', () => {
      ...
      const exportInterfaceLines = lines.filter(line => line.startsWith('export interface'));
      const exportFunctionLines = lines.filter(line => line.startsWith('export function'));
    ...
    it('should handle complex nested formulas', () => {
      const parser = new FormulaParser();
      const complexFormula = '=IF(SUM(A1:A10)>AVERAGE(B1:B10),MAX(C1:C10)*0.1,MIN(D1:D10))';
    ...
    it('should maintain calculation order in generated code', () => {
      ...
      const code = generator.generate(sheets, dependencyGraph, ['Test'], ['Test']);
  ...
  describe('Performance Integration', () => {
    it('should handle large dependency graphs efficiently', () => {
      ...
      for (let i = 1; i < order.length; i++) {
        ...
        expect(currentColumn.charCodeAt(0)).toBeGreaterThan(previousColumn.charCodeAt(0));
      }
    });
  });
});